...
std::vector<std::string> data_get_dead(data_t* self)
{
    ...
    for (expiration_t* e = static_cast<expiration_t*>(zhashx_first(self->assets)); e != NULL;
         e               = static_cast<expiration_t*>(zhashx_next(self->assets))) {
        ...
        if (expiration_get(e) <= now_sec) {
            ...
            dead.emplace_back(static_cast<char*>(asset_name));
            // assert(zlistx_add_start(dead, asset_name));
            // zlistx_add_start(dead, asset_name);

            logInfo("DEAD IN LOOP {} ==========>", dead.size()); // 0 after send alarm
        }
    }
